...
def footprint_satisfies_requirements(
    footprint: tuple[float, float, float, float],
    requirements: PlacementRequirements,
    surface_masks: Mapping[str, np.ndarray],
    world_size: tuple[float, float],
) -> bool:
    """Apply profile-driven hard constraints to a complete XY footprint."""
    # Callers in both placement paths pass prepared masks.  Re-normalize only
    # when a direct API user supplies planner aliases; avoiding a full-raster
    # copy/derivation for every candidate keeps synthetic filtering bounded.
    if all(canonical_surface_name(name) == str(name) for name in surface_masks):
        masks = surface_masks
    else:
        masks = prepare_surface_masks(surface_masks)
    forbidden = set(requirements.forbidden_support_surfaces)
    if requirements.requires_dry_support:
        forbidden.add("water")
    if requirements.avoid_structural_exclusion:
    ...
    if any(
        name in masks and footprint_intersects_mask(footprint, masks[name], world_size)
        for name in forbidden
    ):
        return False

    required = [masks[name] for name in requirements.required_support_surfaces if name in masks]
    if requirements.required_support_surfaces and (
        not required or not any(footprint_intersects_mask(footprint, mask, world_size) for mask in required)
    ):
        return False

    allowed = [masks[name] for name in requirements.allowed_support_surfaces if name in masks]
    if requirements.allowed_support_surfaces and (
        not allowed or not any(footprint_intersects_mask(footprint, mask, world_size) for mask in allowed)
    ):
        return False
    return True
