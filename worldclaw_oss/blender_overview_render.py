...
def _ground_y(scene, x, z):
    ...
    hit, _, _, _ = bvh.ray_cast(Vector((x, 10000.0, z)), Vector((0.0, -1.0, 0.0)), 20000.0)
    return float(hit.y) if hit is not None else 0.0
...
def _clamp_anchor_to_terrain(scene, anchor, margin=500.0):
    ...
    min_z, max_z = min(p.z for p in points) + margin, max(p.z for p in points) - margin
    return Vector((min(max(anchor.x, min_x), max_x), anchor.y, min(max(anchor.z, min_z), max_z)))
...
def _forest_anchor(scene):
    ...
    if terrain is not None:
        ...
        anchor = Vector((center_x, subject.matrix_world.translation.y, center_z))
    else:
        anchor = subject.matrix_world.translation.copy()
    anchor = _clamp_anchor_to_terrain(scene, anchor, margin=80.0)
    return anchor, _ground_y(scene, anchor.x, anchor.z)
...
def add_forest_water_surface(scene):
    ...
    verts = [(anchor.x, water_level, anchor.z)]
    for ring in range(1, radial_steps + 1):
        ...
        for index in range(angular_steps):
            ...
            z = (depth * 0.5) * radius * math.sin(angle)
            ripple = 0.12 * math.sin(x * 0.45) * math.cos(z * 0.38)
            verts.append((anchor.x + x, water_level + ripple, anchor.z + z))
    faces = []
    for index in range(angular_steps):
    ...
    return {"enabled": True, "type": "procedural_wavy_water_surface", "anchor": [float(anchor.x), float(ground), float(anchor.z)], "dimensions": [width, depth]}
...
def add_presentation_proxy(scene, run_name):
    ...
    if "castle" in lowered:
        ...
        ground = _ground_y(scene, anchor.x, anchor.z)
        ...
        _proxy_cube("Presentation_Castle_Courtyard", (anchor.x, ground + 0.8, anchor.z), (34.0, 1.6, 28.0), path)
        _proxy_cube("Presentation_Castle_Keep", (anchor.x, ground + 9.0, anchor.z), (12.0, 16.0, 12.0), stone)
        _proxy_cone("Presentation_Castle_KeepRoof", (anchor.x, ground + 19.0, anchor.z), 9.0, 7.0, roof)
        for dx, dz in ((-12, -9), (12, -9), (-12, 9), (12, 9)):
            _proxy_cube("Presentation_Castle_Tower", (anchor.x + dx, ground + 8.0, anchor.z + dz), (5.0, 15.0, 5.0), stone)
            _proxy_cone("Presentation_Castle_TowerRoof", (anchor.x + dx, ground + 18.0, anchor.z + dz), 4.0, 5.0, roof)
        return {"enabled": True, "type": "castle_low_poly_proxy", "anchor": [float(anchor.x), float(ground), float(anchor.z)]}
    if "desert" in lowered:
        ...
        _proxy_cube("Presentation_Desert_Palace", (anchor.x, ground + 8.0, anchor.z), (40.0, 14.0, 30.0), wall)
        _proxy_cone("Presentation_Desert_Dome", (anchor.x, ground + 18.0, anchor.z), 14.0, 10.0, roof, vertices=16)
        for dx, dz in ((-26, -18), (26, -18), (-26, 18), (26, 18)):
            _proxy_cube("Presentation_Desert_Block", (anchor.x + dx, ground + 5.0, anchor.z + dz), (16.0, 10.0, 14.0), wall)
        return {"enabled": True, "type": "desert_city_low_poly_proxy", "anchor": [float(anchor.x), float(ground), float(anchor.z)]}
...
def settle_assets_on_terrain(scene):
    ...
    for obj in scene.objects:
        ...
        hit, _, _, _ = terrain_bvh.ray_cast(Vector((center.x, 10000.0, center.z)), Vector((0.0, -1.0, 0.0)), 20000.0)
        if hit is None:
            continue
        obj.location.y += float(hit.y - min_y + 0.5)
        moved += 1
    return moved
...
def asset_focus_views(scene):
    ...
    horizontal_radius = max((local_hi.x - local_lo.x), (local_hi.z - local_lo.z), 12.0) * 0.5
    vertical_radius = max(local_hi.y - local_lo.y, 8.0)
    views = []
    for offset in (Vector((0.0, 0.0, 0.0)), Vector((10.0, 0.0, 8.0)), Vector((-8.0, 0.0, 10.0)), Vector((6.0, 0.0, -10.0))):
...
def configure_scene(scene, minimum, maximum, run_name):
    ...
    # Blender uses Y-up after glTF import. Forest uses a fixed-height circular
    # orbit in the X-Z ground plane; the first view is aligned with the X axis.
    orbit = None
    if "forest" in run_name.lower():
        target, depth = views[0]
        radius = max(150.0, depth * 1.08)
        orbit_height = max(108.0, radius * 0.72)
        orbit_end_frame = 144
        ...
        for frame in frames:
            ...
            if frame <= orbit_end_frame:
                ...
                target_path = target + Vector((8.0 * progress, -2.0 * progress, -6.0 * progress))
                position = Vector((target_path.x + radius * math.cos(angle), target_path.y + height, target_path.z + radius * math.sin(angle)))
                orbit_end_position = position.copy()
            else:
                progress = (frame - orbit_end_frame) / float(288 - orbit_end_frame)
                start = orbit_end_position
                forward = Vector((target.x - start.x, 0.0, target.z - start.z)).normalized()
                # Forward fly-through: approach and pass the salient region
                # while lowering the camera and easing the look target.
                end = target + forward * 22.0 + Vector((0.0, 28.0, 0.0))
                position = start.lerp(end, progress)
                target_path = target + Vector((8.0, -2.0, -6.0)) * progress
            camera.location = position
    ...
    directions = [
        Vector((0.85, elevation, -0.95)),
        Vector((-0.95, elevation, -0.75)),
        Vector((-0.8, elevation, 0.95)),
        Vector((0.9, elevation, 0.8)),
        Vector((0.85, elevation, -0.95)),
    ]
...
def main():
    ...
    metadata = {
        ...
        "coordinate_system": "Blender Y-up after glTF import",
